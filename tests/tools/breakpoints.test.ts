...
describe('get_breakpoints tool', () => {
  ...
  it('reports crit cap status correctly', async () => {
    vi.mocked(mockBridge.send).mockResolvedValueOnce({
      ok: true,
      data: { crit_chance: 100, crit_capped: true, hit_chance: 95, fire_res: 75, fire_res_capped: true, cold_res: 50, cold_res_capped: false, lightning_res: 75, lightning_res_capped: true, chaos_res: -60 },
    });
