...
function SkillGroup({ g }: { g: SocketGroup }) {
  ...
  return (
    <div style={{ marginBottom: 8, opacity: g.enabled ? 1 : 0.5 }}>
      ...
      {actives.map((x, i) => (
        <div key={`a${i}`} style={{ paddingLeft: 8, color: g.is_main ? '#d9b45b' : '#cdd3dd' }}>
          {x.name}
        </div>
      ))}
      {supports.map((x, i) => (
...
export function SummaryPanel({ summary }: { summary: BuildSummary }) {
  ...
  const d = dps as { full_dps?: number }
